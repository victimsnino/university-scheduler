...
def _get_indexes_of_timeslots_by_filter(variables, corpus = None, room = None, timeslot = None, lesson = None, group_id = None, type = None, teacher_id = None):
    search = r'.*'
...
class Solver:
            ...
            for corpus_i in self.university.corpuses:
                for ith in range(len(container)):
                    corpus_tracker_index = [self.model.variables.add(obj=[0],
                                                                    ...
                                                                    ub=[1],
                                                                    types=[self.model.variables.type.integer],
                                                                    names=[format_out % ( corpus_i, ith)])[0]]

                    lections_indexes = eval('_get_indexes_of_timeslots_by_filter(self.model.variables, corpus = corpus_i, %s=ith)' % column)
        ...
        for container, _, column in self.__get_groups_teachers_list():
            for ith in range(len(container)):
                for timeslot in range(time_slots_per_day):
                    _add_constraint(self.model, eval('_get_indexes_of_timeslots_by_filter(self.model.variables, timeslot=timeslot, %s=ith)' % column), '<=', 1)
    ...
    def __constraint_ban_changing_corpus_for_groups_or_teachers_during_day(self):
        for container, format_out, _ in self.__get_groups_teachers_list():
            for ith in range(len(container)):
                indexes = []
                for corpus_i in self.university.corpuses:
                    indexes += _get_indexes_by_name(self.model.variables, format_out % ( corpus_i, ith))
    ...
    def __parse_output_and_create_schedule(self):
        ...
        for i in range(len(values)):
            ...
            print('Corpus %s Room %s TS %s Lesson %s Type: %s Teacher: %s GroupIDs %s' % 
            (parsed[0], parsed[1], parsed[2], parsed[3], parsed[5].split('.')[1], self.university.teachers[int(parsed[6])], parsed[4]))
